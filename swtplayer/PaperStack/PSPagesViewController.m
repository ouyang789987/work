...
- (void)deviceDidRotateWithPageOrientation:(PSPagesViewPageOrientation)orientation 
{
    NSLog(@"deviceDidRotateWithOrientation:%d",orientation);
...
- (void)adjustGLView
{
    ...
    NSLog(@"GLViewFrame: %f %f %f %f", self.glView.frame.origin.x, self.glView.frame.origin.y, self.glView.frame.size.width, self.glView.frame.size.height );
}
...
- (void)pageCurlBeganWithPoint:(CGPoint)point
{
    ...
    NSLog(@"SP: %f %f", page.SP.x, page.SP.y );
}
